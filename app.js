...
async function renderCalendar(){
  ...
  for(let d=1;d<=totalDays;d++){
    ...
    const dateStr=formatDate(stateYear,stateMonth+1,d);
  ...
  currentDetailUnsub = docRef.onSnapshot(snap=>{
    ...
    if(items.length===0){
      ...
      items.forEach(item=>{
        ...
        nameSpan.innerText=item.name; if(item.allergy) nameSpan.classList.add('allergy');
...
addFoodBtn.addEventListener('click', async ()=>{
  const name=foodInput.value.trim(); if(!name) return alert('請輸入食物名稱');
